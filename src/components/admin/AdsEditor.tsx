  ...
  return (
    <div className="space-y-4">
      <div className="card grid grid-cols-1 gap-3 p-4 sm:grid-cols-2">
        ...
        <input className="input" placeholder="آدرس تصویر" value={form.imageUrl} onChange={(e) => setForm({ ...form, imageUrl: e.target.value })} />
        <input className="input" placeholder="لینک (اختیاری)" value={form.linkUrl} onChange={(e) => setForm({ ...form, linkUrl: e.target.value })} />
        ...
        <button className="btn-primary sm:col-span-2" onClick={add} disabled={busy}>افزودن تبلیغ</button>
      </div>
          ...
          <div key={a.id} className="card overflow-hidden">
            ...
            <div className="space-y-2 p-3">
              ...
              <div className="flex items-center justify-between">
                ...
                <button className="text-xs text-red-600 hover:underline" onClick={() => remove(a.id)}>حذف</button>
              </div>
            </div>
          </div>
        ))}
        {ads.length === 0 && <div className="col-span-full text-gray-500">تبلیغی نیست.</div>}
      </div>
    </div>
  );
}
